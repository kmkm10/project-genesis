...
def signup():
    if request.method == 'POST':
        username, password, conn = request.form['username'], request.form['password'], get_db_connection()
        error = None
        ...
        if error is None:
            ...
            user_cursor.execute('INSERT INTO users (username, password) VALUES (?, ?)', (username, generate_password_hash(password)))
            new_user_id = user_cursor.lastrowid
            conn.execute(
                '''INSERT INTO players (user_id, last_update_time, research_points, money, total_rp_earned, 
                   ...
                   VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)''',
                (new_user_id, time.time(), 10.0, 50.0, 0.0, 1.0, 0.5, 0, '[]', None, 
                 json.dumps({k: 0 for k in FACILITIES}), 0, 0, 0, 0, time.time())
            )
            conn.commit()
            conn.close()
            flash('登録が完了しました。ログインしてください。')
            return redirect(url_for('login'))
        flash(error); conn.close()
...
def get_gamestate():
    ...
    player_data, log = update_player_state(user_id)
    ...
    if player_data['total_rp_earned'] >= threshold:
        ...
        conn = get_db_connection(); conn.execute('UPDATE players SET civilization_level = ? WHERE user_id = ?', (next_level, user_id)); conn.commit(); conn.close()
...
def perform_action():
    user_id = session['user_id']
    _, _ = update_player_state(user_id)
    conn = None
    try:
        ...
        if action == 'start_research':
            ...
            if not tech or player['research_points'] < tech['cost'] or (player['researching_tech'] is not None):
                return jsonify({'success': False, 'message': '研究を開始できません。'})
            new_rp = player['research_points'] - tech['cost']
            new_researching = json.dumps((item_id, time.time(), tech['time']))
            conn.execute('UPDATE players SET research_points = ?, researching_tech = ? WHERE user_id = ?', (new_rp, new_researching, user_id))
        
        elif action == 'upgrade_facility':
            facility = FACILITIES.get(item_id)
            levels = json.loads(player['facility_levels'])
            ...
            conn.execute('UPDATE players SET money=?, facility_levels=?, rp_per_second=?, money_per_second=? WHERE user_id=?',
                         (new_money, json.dumps(levels), new_rp_rate, new_money_rate, user_id))
        else:
            return jsonify({'success': False, 'message': '未知のアクションです。'}), 400
...
def genesis_shift():
    ...
    try:
        conn = get_db_connection()
        player = conn.execute('SELECT * FROM players WHERE user_id = ?', (user_id,)).fetchone()
        if 'astronomy' not in json.loads(player['unlocked_technologies']):
            return jsonify({'success': False, 'message': '転生の条件を満たしていません。'})
        ...
        conn.execute('''
            UPDATE players SET last_update_time=?, research_points=10.0, money=50.0, total_rp_earned=0.0,
                rp_per_second=?, money_per_second=?, civilization_level=0, unlocked_technologies='[]', 
                researching_tech=null, facility_levels=?, evolution_points=?, genesis_shifts=?, run_start_time=?
            WHERE user_id=?
        ''', (time.time(), base_rp, base_money, json.dumps({k: 0 for k in FACILITIES}), 
              player['evolution_points'] + ep_gain, player['genesis_shifts'] + 1, time.time(), user_id))
        conn.commit()
        return jsonify({'success': True, 'message': f'ジェネシス・シフト発動！ {ep_gain} EPを獲得しました！'})
    finally:
        if conn: conn.close()
...
def purchase_permanent_upgrade():
    ...
    try:
        ...
        conn.execute(f'UPDATE players SET evolution_points = ?, {target_column} = ? WHERE user_id = ?',
...
def format_data_for_frontend(player_data, log=[]):
    ...
    unlocked = json.loads(player_dict['unlocked_technologies'])
    ...
    dashboard_stats = {
        'time_elapsed_this_run': time.time() - player_dict.get('run_start_time', time.time()),
        'total_rp_this_run': player_dict['total_rp_earned'],
        'unlocked_tech_count': len(unlocked),
        'total_tech_count': len(TECHNOLOGIES),
        'total_facility_levels': sum(facilities.values())
    }

    perm_upgrades_info = []
    for up_id, up_data in PERMANENT_UPGRADES.items():
        level = player_dict[up_data['target_column']]
        cost = int(up_data['base_cost'] * (up_data['cost_increase_factor'] ** level))
        effect = (level + 1) * up_data['effect_per_level']
        perm_upgrades_info.append({
            'id': up_id, 'name': up_data['name'], 'level': level,
            'description': up_data['description_template'].format(effect=effect),
            'cost': cost
        })
    
    perm_rp_bonus = player_dict['perm_bonus_rp_level'] * PERMANENT_UPGRADES['rp_bonus']['effect_per_level']
    perm_money_bonus = player_dict['perm_bonus_money_level'] * PERMANENT_UPGRADES['money_bonus']['effect_per_level']

    return {
        'research_points': player_dict['research_points'], 'money': player_dict['money'],
        'rp_per_second': player_dict['rp_per_second'] + perm_rp_bonus,
        'money_per_second': player_dict['money_per_second'] + perm_money_bonus,
        'unlocked_technologies': unlocked, 'researching_tech': researching,
        'available_technologies': [{'id': tid, **tech} for tid, tech in TECHNOLOGIES.items() if tid not in unlocked and (not researching or tid != researching[0]) and all(req in unlocked for req in tech['req'])],
        'facilities': [{'id': fid, 'level': facilities.get(fid, 0), 'cost': fac['base_cost'] * (1.5 ** facilities.get(fid, 0)), **fac} for fid, fac in FACILITIES.items()],
        'civilization': CIVILIZATION_LEVELS[player_dict['civilization_level']],
        'evolution_points': player_dict['evolution_points'], 'genesis_shifts': player_dict['genesis_shifts'],
        'permanent_upgrades': perm_upgrades_info,
...
def update_player_state(user_id):
    ...
    try:
        ...
        perm_rp_bonus = player_data['perm_bonus_rp_level'] * PERMANENT_UPGRADES['rp_bonus']['effect_per_level']
        perm_money_bonus = player_data['perm_bonus_money_level'] * PERMANENT_UPGRADES['money_bonus']['effect_per_level']
        
        player_data['research_points'] += (player_data['rp_per_second'] + perm_rp_bonus) * elapsed
        player_data['total_rp_earned'] += (player_data['rp_per_second'] + perm_rp_bonus) * elapsed
        ...
        if researching and now - researching[1] >= researching[2]:
            ...
            player_data['unlocked_technologies'] = json.dumps(unlocked)
            # ▼▼▼【バグ修正の核心部分】▼▼▼
            player_data['researching_tech'] = None # json.dumps(None)ではなく、本物のNoneをセット

        player_data['last_update_time'] = now
        conn.execute('UPDATE players SET last_update_time=?, research_points=?, money=?, total_rp_earned=?, researching_tech=?, unlocked_technologies=? WHERE user_id=?',
                     (now, player_data['research_points'], player_data['money'], player_data['total_rp_earned'], 
                      player_data['researching_tech'], player_data['unlocked_technologies'], user_id))
        conn.commit()
        return player_data, log
    finally:
        if conn: conn.close()
