...
def signup():
    if request.method == 'POST':
        ...
        if error is None:
            ...
            cur.execute(
                f'''INSERT INTO players (user_id, last_update_time, research_points, money, total_rp_earned, 
                ...
                (new_user_id, time.time(), 10.0, 50.0, 0.0, 1.0, 0.5, 0, '[]', None, 
                 initial_facilities, 0, 0, 0, 0, time.time())
            )
            conn.commit()
...
def get_gamestate():
    ...
    if player_data['total_rp_earned'] >= threshold:
        ...
        cur.execute(f'UPDATE players SET civilization_level = {ph} WHERE user_id = {ph}', (next_level, session['user_id']));
...
def format_data_for_frontend(player_data, log=[]):
    ...
    facilities = json.loads(player_data['facility_levels'])
...
def update_player_state(user_id):
    ...
    try:
        ...
        if researching and now - researching[1] >= researching[2]:
            ...
            player_data['researching_tech'] = None
